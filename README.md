...
- **工作流画布（`/canvas`）把"一次点击两步"变成可保存的链**：节点＝已验证的模板（不是 ComfyUI 的节点，
  ...
  同一条路径。**已知不足**：`ref2-ref4` 只能填文件、不能连线（一次运行只带一张参考图）；
  高清后的 3328×4864 当视频首帧会被 H3 拉成 480×864
  （不裁切，比例不同会压扁），要保原图构图就跳过中间那一跳直接用人物图。

- **链上的每一步自己决定失败了怎么办**：画布步骤抽屉第 3 个页签「这一步」（列表页的抽屉没有这个页签，
  单次运行没有"往下"可跳），三格全在界面上选——执行次数（1–8，就是循环）、重试次数（0–3）、
  失败时（停下整条链 / 跳过它继续往下）；下面一行直接算给你代价：
  「这一步最多提交 9 次（3 遍 × 每遍最多 3 次尝试），单次上限 15 分钟」，上限取自模板自己的 timeout。
  节点卡上非默认的开关变成角标（`3 遍` / `失败重试 2` / `失败可跳过`），不开抽屉也看得见。
  循环的每一遍、重试的每一次**都换 seed 再提**：同参数的重提会被引擎当缓存命中（实测 `seconds=0.0`、
  产物文件名不前进），不换 seed 等于白跑。每一遍的产物各自记在自己的运行上，下游那根线多一格
  「取第几遍」（线上标签写成 `r2·#1`）。跳过会往下传：接在它下游的步骤因为「没有输入」一并跳过，
  写明是哪一步第几遍害的，整条链收尾成 `部分完成` 而不是假装全绿。
  还有一种线是**判断**：卡片左右下角那对琥珀色虚线方格之间拖出来的线不带任何图片，只带一个条件——
  点它，面板上问「什么时候走这条路」：上游失败或被跳过时 / 上游成功时 / 不管上游怎样（默认第一条，
  线上标 `fail`）。带图的线（黑实线，标 `#1`、`r2·#1`）和条件线不能混接，两端 socket 差 57–181px，
  拖错拿不到（实测：琥珀→琥珀 2→3 条线，图→琥珀 仍是 3 条）。既接了某步的产物又设成"它失败时才走"
  在保存时就拒（这条组合永远不可能成立）。
  工具条上多了 ■ 取消：正在跑的那一步向引擎要回 GPU（`POST /queue` delete + `/interrupt`，
  这引擎只认这一套），后面的步骤不再提交。真引擎上量过：循环 2 遍 = seeds 42 / 715368504、
  两张不同的图（`s1_00001_.png`、`s1_00002_.png`）、下游按连线取到第 2 遍那张，整条 18.2s；
  2048² 图标第一步跑到中途被掐掉（seed 90096632 → error）后重试换成 618607937，20.3s 出图、整条 24.2s；
  两步链在第一步执行中取消，2.7s 收尾 `cancelled`、引擎队列 2.65s 内空掉、第二步压根没变成任务，
  随后再提一个任务 4.0s 正常完成——没有僵尸作业占着显卡；真失败（引擎 400）那次的三张卡片实测是
...
最近一次运行的状态还在。某一步想多跑几遍或扛住失败：点卡片 → 第 3 个页签「这一步」，执行次数 / 重试次数 / 失败时
三格都在那里（列表页的抽屉没有这个页签，单次运行没有"往下"可跳）。要分叉就从卡片右下角的琥珀方格拖到另一张卡片左下角的琥珀方格。跑起来之后工具条上的 ■ 取消 会亮，
点它就是在请求取消——卡片上那行状态会先停在「进行中」一两秒，等那一步的任务行落定才改口。
一条链的收尾有四种：`已完成`、`失败`（某一步试够了还没让它跳）、`部分完成`（有步骤被跳过、
其余照常跑完）、`已取消`。
