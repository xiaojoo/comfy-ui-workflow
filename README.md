...
cd server && .venv/Scripts/python.exe -m pytest tests -q                    # 6 项验收
```
...
- 设计稿里的「新建工作流」、卡片 ⋯ 菜单、通知铃/用户头像、FLUX.2 与 SDXL 那几张卡片
  后端一概没有，因此没做。
- 卡片上的缩略图是 `Workflows.vue` 里写死的三条，文生视频与 3D 两条没有配图；
  预览轨道里的产物图是真的（全部运行，选中卡片后只看这条工作流的）。
- 生成与门禁在界面上还是两次动作，没串成「生成→抠图→矢量化→门禁」一键。
- 设计稿的 6 类里我们只有企业图标这一条实测链路（外加视频与 3D 两条）；
...
- 视频**单条上限约 19 秒**（20 分钟预算内）；要更长只能降步数/降分辨率或分段拼接，
  滑窗 context window 已实测为负结果。**人物一致性仍没有可分辨指标**，
  现在只有"跨帧平均跳变"这种粗代理，它答不了"还是不是同一个人"。
